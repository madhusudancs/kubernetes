...
func newReplicaSet(replicas int, selectorMap map[string]string) *extensions.ReplicaSet {
	rs := &extensions.ReplicaSet{
		...
		Spec: extensions.ReplicaSetSpec{
			Replicas: replicas,
			Selector: &extensions.PodSelector{MatchLabels: selectorMap},
...
func TestSyncReplicationControllerDoesNothing(t *testing.T) {
	...
	manager.syncReplicaSetController(getKey(rsSpec, t))
...
func TestSyncReplicationControllerCreates(t *testing.T) {
	...
	fakePodControl := controller.FakePodControl{}
	manager.podControl = &fakePodControl
	manager.syncReplicaSetController(getKey(rs, t))
...
func TestPodControllerLookup(t *testing.T) {
	manager := NewReplicaSetController(client.NewOrDie(&client.Config{Host: "", GroupVersion: testapi.Default.GroupVersion()}), controller.NoResyncPeriodFunc, BurstReplicas)
	manager.podStoreSynced = alwaysReady
	testCases := []struct {
	...
	for _, c := range testCases {
		...
		if rs := manager.getPodReplicaSet(c.pod); rs != nil {
			if c.outRSName != rs.Name {
				t.Errorf("Got controller %+v expected %+v", rs.Name, c.outRSName)
			}
		} else if c.outRSName != "" {
			t.Errorf("Expected a controller %v pod %v, found none", c.outRSName, c.pod.Name)
		}
	}
}
...
func TestWatchControllers(t *testing.T) {
	...
	var testControllerSpec extensions.ReplicaSet
	received := make(chan string)
	...
	manager.syncHandler = func(key string) error {
		...
		if !api.Semantic.DeepDerivative(rsSpec, testControllerSpec) {
			t.Errorf("Expected %#v, but got %#v", testControllerSpec, rsSpec)
		}
		close(received)
		return nil
	}
...
func TestWatchPods(t *testing.T) {
	...
	labelMap := map[string]string{"foo": "bar"}
	testControllerSpec := newReplicaSet(1, labelMap)
	manager.rsStore.Store.Add(testControllerSpec)
	received := make(chan string)
	...
	manager.syncHandler = func(key string) error {
		...
		rsSpec := obj.(*extensions.ReplicaSet)
		if !api.Semantic.DeepDerivative(rsSpec, testControllerSpec) {
			t.Errorf("\nExpected %#v,\nbut got %#v", testControllerSpec, rsSpec)
		}
		close(received)
		return nil
	}
	...
	pods := newPodList(nil, 1, api.PodRunning, labelMap, testControllerSpec)
...
func TestUpdatePods(t *testing.T) {
	...
	testControllerSpec1 := newReplicaSet(1, labelMap1)
	manager.rsStore.Store.Add(testControllerSpec1)
	testControllerSpec2 := *testControllerSpec1
	labelMap2 := map[string]string{"bar": "foo"}
	testControllerSpec2.Spec.Selector = &extensions.PodSelector{MatchLabels: labelMap2}
	testControllerSpec2.Name = "barfoo"
	manager.rsStore.Store.Add(&testControllerSpec2)

	// Put one pod in the podStore
	pod1 := newPodList(manager.podStore.Store, 1, api.PodRunning, labelMap1, testControllerSpec1).Items[0]
	pod2 := pod1
...
func TestRSManagerNotReady(t *testing.T) {
	...
	manager.syncReplicaSetController(rsKey)
	validateSyncReplication(t, &fakePodControl, 0, 0)
