...
func NewReplicaSetController(kubeClient client.Interface, resyncPeriod controller.ResyncPeriodFunc, burstReplicas int) *ReplicaSetController {
	...
	rsc.rsStore.Store, rsc.rsController = framework.NewInformer(
		...
		framework.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(old, cur interface{}) {
				...
				rsc.enqueueController(cur)
			},
			// This will enter the sync loop and no-op, because the controller has been deleted from the store.
			// Note that deleting a controller immediately after scaling it to 0 will not work. The recommended
			// way of achieving this is by performing a `stop` operation on the controller.
			DeleteFunc: rsc.enqueueController,
		},
	)
	...
	rsc.syncHandler = rsc.syncReplicaSetController
...
// getPodReplicaSet returns the controller managing the given pod.
...
func (rsc *ReplicaSetController) enqueueController(obj interface{}) {
	...
	// by querying the store for all controllers that this ReplicaSet overlaps, as well as all
	// controllers that overlap this ReplicaSet, and sorting them.
	rsc.queue.Add(key)
}
...
func (rsc *ReplicaSetController) manageReplicas(filteredPods []*api.Pod, rs *extensions.ReplicaSet) {
	...
	if diff < 0 {
		...
		for i := 0; i < diff; i++ {
			go func() {
				...
				if err := rsc.podControl.CreatePods(rs.Namespace, rs.Spec.Template, rs); err != nil {
					...
					glog.V(2).Infof("Failed creation, decrementing expectations for controller %q/%q", rs.Namespace, rs.Name)
					rsc.expectations.CreationObserved(rsKey)
		...
		for i := 0; i < diff; i++ {
			go func(ix int) {
				defer wait.Done()
				if err := rsc.podControl.DeletePod(rs.Namespace, filteredPods[ix].Name); err != nil {
					// Decrement the expected number of deletes because the informer won't observe this deletion
					glog.V(2).Infof("Failed deletion, decrementing expectations for controller %q/%q", rs.Namespace, rs.Name)
					rsc.expectations.DeletionObserved(rsKey)
					util.HandleError(err)
				}
			}(i)
...
// syncReplicaSetController will sync the ReplicaSet with the given key if it has had its expectations fulfilled,
// meaning it did not expect to see any more of its pods created or deleted. This function is not meant to be
// invoked concurrently with the same key.
func (rsc *ReplicaSetController) syncReplicaSetController(key string) error {
	startTime := time.Now()
	defer func() {
	...
	if !exists {
		glog.Infof("ReplicaSet Controller has been deleted %v", key)
